...
public class Teste {
	...
	public static void adicionarIgreja(Scanner scan) {
		...
		igreja.setCondicaoPastorIgreja(
				leInformacaoInt("CondiÃ§Ã£o (1.Efetivo; 2. Auxiliar; 3.Evangelista ou 4.MissionÃ¡rio): ", scan));
	...
	// protected static Date leInformacaoData(String msg, Scanner scan) throws
	// ParseException{
	...
	private static void listarAvisos() {
		...
		for (AvisoVO aviso : avisos) {
			System.out.println(aviso.getIdAviso() + "\t" + aviso.getAssuntoAviso() + "\t" + aviso.getAssuntoAviso());
		}
	}
	...
	private static void listarVisitantes() {
		...
		for (VisitanteVO vis2 : visitantes) {
			...
			System.out.println(vis2.getIdPessoa() + "\t" + vis2.getNomePessoa() + "\t" + vis2.getLogradouroPessoa()
					+ "\t" + vis2.getBairroPessoa());
		}
	}
